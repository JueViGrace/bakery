        ...
        route("users") {
            ...
            get("/{id}") {
                val idParam = call.parameters["id"]
                    ?: return@get call.respond(DefaultHttpResponse.badRequest<Nothing>("Parameter id must be provided"))

                if (idParam.map { it.isDigit() }.contains(false)) {
                    return@get call.respond(
                        DefaultHttpResponse.badRequest<Nothing>("Parameter id must be a valid number")
                    )
                }
            ...
            put("/{id}") {
                val idParam = call.parameters["id"]
                    ?: return@put call.respond(DefaultHttpResponse.badRequest<Nothing>("Parameter id must be provided"))
            ...
            delete("/{id}") {
                val idParam = call.parameters["id"]
                    ?: return@delete call.respond(
                        DefaultHttpResponse.badRequest<Nothing>("Parameter id must be provided")
                    )
